...
import com.sena.dms.spring.model.data.GalleriesInfo;
import com.sena.dms.spring.model.data.UserSession;
import com.sena.dms.spring.model.entity.ImageDetails;
import com.sena.dms.spring.service.GalleriesService;
import com.sena.dms.spring.util.WebUtil;
import com.sena.dms.spring.validator.GalleriesInfoValidator;
...
public class GalleriesController {
	...
	public void showImage(@PathVariable("user_id") String user_id, HttpServletResponse response, HttpServletRequest request, Model model) {
		try {
			ImageDetails galleryDetails = galleriesService.getFileById(user_id, "1");
			response.setContentType("image/jpeg, image/jpg, image/png, image/gif");
			if (galleryDetails != null) {
				response.getOutputStream().write(galleryDetails.getImage_data());
			} else {
